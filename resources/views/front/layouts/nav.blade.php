...
<ul class="mainmenu">
    ...
    @foreach($navs as $nav)
        ...
        <li class="{{ count($childCategories) > 0 ? 'dropdown-new' : '' }}">
            <a href="{{ url('shop?slug=' . $nav->slug) }}" class="{{ $__env->yieldContent('nav_active_category') == $nav->slug ? 'active' : '' }}">{{ $nav->name }} {{ count($childCategories) > 0 ? '&#9662;' : '' }}</a>
